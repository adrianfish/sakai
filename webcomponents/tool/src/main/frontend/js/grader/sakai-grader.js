    ...
    return html`
      ...
      ${this.gradeScale === PASS_FAIL_GRADE_TYPE ? html`
        <select id="pass-fail-selector"
          ...
          <option value="ungraded"
              .selected=${submitter ? submitter.grade.match(/^ungraded$/i) : this.submission.grade.match(/^ungraded$/i)}>
            ${this.i18n.ungraded}
          </option>
          <option value="pass"
              .selected=${submitter ? submitter.grade.match(/^pass$/i) : this.submission.grade.match(/^pass$/i)}>
            ${this.assignmentsI18n.pass}
          </option>
          <option value="fail"
              .selected=${submitter ? submitter.grade.match(/^fail$/i) : this.submission.grade.match(/^fail$/i)}>
      ...
      ${this.gradeScale === CHECK_GRADE_TYPE ? html`
        <input id="check-grade-input"
                ...
                value=${GRADE_CHECKED}
                .checked=${submitter ? (submitter.overridden && submitter.grade === GRADE_CHECKED) : this.submission.grade === GRADE_CHECKED}>
        </input>
