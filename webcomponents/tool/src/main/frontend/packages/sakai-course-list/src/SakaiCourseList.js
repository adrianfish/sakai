...
export class SakaiCourseList extends SakaiElement {
  ...
  _filter() {
    ...
    if (this._currentFilter === "pinned") {
      filteredSites = [ ...filteredSites.filter(s => s.pinned) ];
    }
  ...
  render() {
    ...
    return html`
      <div class="cl-course-list-controls">
        <div class="filter">
          <select aria-label="${this._i18n.course_filter_label}" @change=${this._siteFilterChanged} .value=${this._currentFilter}>
            <option value="all">${this._i18n.view_all_sites}</option>
            <option value="pinned">${this._i18n.pinned}</option>
      ...
      <div>
        ${this._displayedSites.map(card => html`
          <sakai-course-card .courseData=${card}></sakai-course-card>
        `)}
      </div>
    `;
  }
